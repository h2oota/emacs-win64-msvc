  ...
  (let ((proc (apply 'start-process name buffer program args)))
	...
	(set-process-coding-system proc
      ...
      (set-process-coding-system
       proc migemo-coding-system migemo-coding-system))
    proc))
...
(defun migemo-init ()
  ...
  ;; mod-cmigemo begin
  (unless
      (and (featurep 'mod-cmigemo)
           (or *mod-cmigemo-handle*
      ...
      (setq pattern
	    (cond
	     ...
	     (t
	      (save-excursion
		;; mod-cmigemo begin
		(if *mod-cmigemo-handle*
		    (setq pattern  (cmigemo-query *mod-cmigemo-handle* word))
		  ;; !!!WARNING!!! the following block has original indent
...
(defadvice isearch-search-and-update (around migemo-search-ad activate)
  ...
  (let ((isearch-adjusted isearch-adjusted))
    (when (and migemo-isearch-enable-p
	       (not isearch-forward) (not isearch-regexp)
...
This function used with Megemo feature."
  (let ((case-fold-search isearch-case-fold-search)
        (choices (cond (isearch-regexp-function
